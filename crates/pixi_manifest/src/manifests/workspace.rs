...
impl WorkspaceManifestMut<'_> {
    ...
    pub fn remove_workspace_platforms(
        ...
        let known_platform_names = self.known_platform_names();

        // Update Manifest feature platforms
        let feature_names = self.workspace.features.keys().cloned().collect::<Vec<_>>();

        for feature_name in &feature_names {
            self.workspace
                .get_or_insert_feature_mut(feature_name)
                .platforms_mut()
                .retain(|pn| known_platform_names.contains(pn))
        }
    ...
    pub fn add_dependency(
        ...
        for platform_name in to_options(platforms) {
            let selector = platform_name
                .as_ref()
                .map(|p| TargetSelector::Platform(p.clone()));
            match self
                .workspace
...
// Handles the target missing error cases. A `None` platform refers to the
// default (no target selector) entry.
fn handle_missing_target(
    platform: Option<&PixiPlatformName>,
    feature_name: &FeatureName,
    section: &str,
) -> miette::Report {
    let target_path = match platform {
        Some(p) => format!("target.{p}."),
        None => String::new(),
    };
    let help = if feature_name.is_default() {
        format!(r#"Expected target for `{feature_name}`, e.g.: `[{target_path}{section}]`"#)
    } else {
        format!(
            r#"Expected target for `{feature_name}`, e.g.: `[feature.{feature_name}.{target_path}{section}]`"#
        )
    };
    match platform {
        Some(p) => miette!(
            help = &help,
            "No target for feature `{feature_name}` found on platform `{p}`",
        ),
        None => miette!(
            help = &help,
            "No default target for feature `{feature_name}`",
        ),
    }
}
