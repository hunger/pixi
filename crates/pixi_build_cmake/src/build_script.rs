...
/// Renders the CMake file that registers a dependency provider for the
/// discovery run.
///
/// The provider is called before CMake searches for a package. It repeats the
/// search that CMake would have done, records the outcome, and stands in for
/// whatever is missing so that one run reports every package the project
/// looks for rather than stopping at the first one it cannot find.
...
mod test {
    ...
    fn test_build_script(
        ...
        let context = BuildScriptContext {
            ...
            provider_file_lines: provider_file_lines(
