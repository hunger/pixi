...
/// entry on the client. Mirrors [`localise_prefix`]'s symlink-mode
/// idempotency rules but is willing to clobber a real file at
/// `local_path` — that's almost always a stale trampoline written
/// by an earlier non-daemon install of the same exposed name, and
/// refusing to overwrite it would leave the user with a broken
