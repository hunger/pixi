...
!!! tip "Trim it for portability"
    ...
    After installing, `pixi info` reports each environment's **Minimum platform** (the virtual-package requirements some resolved dependency really places on the machine), so you can see which declared guarantees are safe to drop with `pixi workspace platform edit`. These are shown as conda match specs, because a dependency can require a version range or a set of build strings rather than one exact value.
